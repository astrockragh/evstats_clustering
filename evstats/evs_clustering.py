...
max_z = 15
n_z = 10
Ntrials = int(1e5) # number of overdensities to sample
...
mmin = 6 # Minimum halo mass for HMF
...
Nm = 100 # set to 100
mbins = np.arange(6., maxMass+dM/Nm-1e-10, dM/Nm)
...
def trial_minsig(sig_v, mean, Ntrials=10000):
    ...
    k = mean**2/var
    t = var/mean
    x = np.linspace(1e-8, 1-1e-8, Ntrials)
    rand = np.rint(t*gammaincinv(k, x, out = None)) # this is only accurate when sigma_CV>0.05
    return rand
...
def evs_clustering(cv_df, mf = hmf.MassFunction(), V = 1, z = 4, Ntrials = int(1e5)):
    ...
    return phi_maxs.T, smfs, mbins, N_trapz
...
if __name__ == '__main__':
    ...
    for z in tqdm.tqdm(zs, total = len(zs)):
        pm, smfs, mbins, N_trapz = evs_clustering(cv_df = cv_df, V = V_OD, z = z, Ntrials = Ntrials)
